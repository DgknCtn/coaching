-- ============================================================
-- 011_prevent_duplicate_homework_items
-- Bug: create_homework_batch only prevents the same book_test_id
-- appearing twice *within one batch* (homework_items UNIQUE
-- (homework_batch_id, book_test_id)). It does NOT stop the same
-- test being assigned again to the same student in a *separate*
-- batch, creating duplicate open homework items for one test.
--
-- Fix: reject the whole batch (transactional, all-or-nothing) if
-- any item's book_test_id already has a non-terminal homework_item
-- for that student.
--
-- Body is otherwise identical to 010_homework_batch_student_check.sql.
-- ============================================================
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Reject if this student already has an open (pending) homework item
    -- for the same test, in ANY batch — prevents accidental duplicate
    -- assignment of the same test.
    IF EXISTS (
      SELECT 1
