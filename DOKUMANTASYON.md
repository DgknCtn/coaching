...
### R7 / Site Testi 05 — ödev akışa bağlandı, ana temas değişikliği soruluyor

**Son teslim artık akıştan miras alınıyor** (kabul #4, §1). Ödev Planlama'nın sağ panelinde tarih boş bir alan değil; aktif akışın kesinleşmiş kapanışı yazıyor — *"Aktif Haftalık Akış'tan otomatik"* — ve yanında tek işlem var: **Değiştir**. Belgenin cümlesi buydu: *"Öğretmene aynı tarihi tekrar seçtirmek gerekmez."* Akış yoksa alan eskisi gibi elle girilir ve bunun sonucu da yazıyor: ödev bir haftaya bağlanmaz. Kaydedilmiş taslak akışın varsayılanını **ezer** — öğretmen o taslakta tarihi bilinçle değiştirmiş olabilir, sessizce geri almak yaptığı işi silmek olurdu.

**Senaryo B artık görünür.** "Farklı son teslim" seçildiğinde tarihin sonucu anında yazılıyor: aktif haftanın içinde mi kalıyor, yoksa kapanışı aşıp gelecek akışa mı gidiyor. Yayından sonra parti akışa bağlanamadıysa bu da söyleniyor. Önceden sonuç yalnız `console.error`'a gidiyordu: son teslimi kapanışı aşan ödev sessizce akışsız kalıyor, öğretmen "yayınlandı" görüp haftanın toplamına eklendiğini sanıyordu. Yayın **başarılıdır**, bu yüzden hata değil uyarı olarak dönüyor.

**`flowMembership` sessizce SQL'den ayrışmıştı.** TypeScript zaman damgalarını (`getTime`) karşılaştırıyordu, `attach_batch_to_flow` (077:367) ise `due_date > (due_at AT TIME ZONE 'Europe/Istanbul')::DATE` ile **günleri**. Ödevin son teslimi bir DATE, akışın kapanışı saatli bir andır; kapanışı Pazar 10:00 olan bir haftada aynı güne verilen ödev SQL'de bağlanıyor, arayüzde "gelecek hafta" görünüyordu. Fonksiyon gün düzeyine çekildi ve ayrışma `tests/weekly-flow-sql-parity.test.ts` ile kilitlendi — migration metnindeki karşılaştırma biçimi (`::DATE`, saat dilimi, `>` operatörü) okunuyor. İki tarafın aynı soruyu iki farklı cevaplaması, haftanın toplam yükünü ve tempo hesabını doğrudan yalancı yapar.

**Ana temas ertelenince kapanış TAŞINMAZ, SORULUR** (kabul #11, §4). `shouldAskToMoveDue()` 077'de yazılmış ve test edilmişti ama hiçbir yerden çağrılmıyordu; RPC'nin yorumu arayüzün soracağını vaat ediyordu, arayüz sormuyordu. Artık `rescheduleSessionAction` ertelenen oturumun ana temas olup olmadığını `deriveMainContact` ile belirliyor (ikinci bir öncelik listesi yazılmadı — iki ekran farklı hizmeti ana temas sayabilirdi) ve öğretmene soruyor. Yeni kapanış, oturumun yeni saatinden `submission_offset_minutes` düşülerek bulunuyor: online grup dersindeki "ders − 6 saat" kuralı tam olarak bu paydır.

**Özel son teslim seçilmişse soru sorulmuyor**, hatırlatma yapılıyor: *"Haftanın son teslimi özel seçilmiş; ana temas değişikliği bu tarihi değiştirmedi."* Belgenin kendi kuralı — öğretmen o haftanın kapanışını zaten elle kurmuşsa sistem onu ne değiştirir ne de tekrar sorar.

### R7 / Site Testi 05 — Durum Bildirimleri Haftalık Akış'a taşındı (079)

**İki saat birbirinden habersiz işliyordu.** 016 bildirimi tek bir sabite bağlıyordu: son bildirimden `interval_days` (varsayılan 3) gün sonra bir tane daha. Bu saat haftadan hiç haberdar değildi — iki günlük bir akışta 3 gün sonra sorulan bildirim hafta kapandıktan sonra geliyor, on günlük bir akışta ise takvimle konuşuyordu. Daha kötüsü: öğrenci beş gündür hiç teslim yapmamış ve son bildirimini dün vermişse sistem susuyordu, oysa asıl sorulacak an tam orasıydı.

077 bu kuralı TypeScript'te zaten yazmıştı (`checkInDue`) — iki tetikleyici, ikisi de yeterli: akışın ortası geçtiyse (**ritim**) ya da teslim sessizliği eşiği aşıldıysa (**hareket**). Ama fonksiyon **hiçbir yerden çağrılmıyordu**; bildirimi üreten `ensure_student_check_ins` hâlâ yalnız sabiti biliyordu. Yazılmış, test edilmiş ve kullanılmamış bir kural, olmayan bir kuraldır.

**`due_at` artık üç adaydan en erkeni** (`LEAST`): 016'nın sabiti (**taban** — akışı olmayan öğrencide tek ölçü budur ve kaldırılsaydı o öğrencilerde bildirim büsbütün dururdu), akışın orta noktası, ve son teslim + 3 gün. Belge iki tetikleyici için de *"ikisi de yeterli"* diyor; yeterli koşulların birleşimi ilk gerçekleşenin zamanıdır. Her aday `anchor_at`'ten sonra olmak zorunda — aksi hâlde geçmişte kalmış bir orta nokta her sayfa yüklenişinde "vakti geçmiş" bir bildirim doğururdu.

**Bildirimin haftası türetilmiyor, kaydediliyor** (`weekly_flow_id`). Akışın kapanışı sonradan taşınabiliyor (`set_weekly_flow_due`) ve o anda tarih karşılaştırmasıyla bakan her sorgu bildirimi başka bir haftaya kaydırırdı — 077'nin `homework_batches.weekly_flow_id` için verdiği gerekçenin aynısı. Sütun nullable: akış kavramından önceki bildirimleri uydurma bir haftaya bağlamak, olmayan bir geçmişi kaydetmek olurdu.
